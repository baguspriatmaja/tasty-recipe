...
export default {
    ...
    actions: {
        async getRegisterData({ commit, dispatch }, payload) {
            const APIkey = "AIzaSyD32eZkU8G0Anef8H4AcZ86GC4Wq2Hb0Bs";
        ...
        async addNewUser({ commit, state }, payload) {
            try {
                const { data } = await axios.post(`https://vue-js-project-b1fe4-default-rtdb.firebaseio.com/user.json?auth=${state.token}`, payload);
        ...
        async getUser({ commit }, payload) {
            try {
                const { data } = await axios.get(`https://vue-js-project-b1fe4-default-rtdb.firebaseio.com/user.json`)
                for (let key in data) {
